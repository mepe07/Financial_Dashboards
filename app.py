"""
# Dashboard Financeiro PS2

Este é o módulo principal da aplicação Shiny. Responsável por inicializar a interface,
carregar os dados iniciais e definir toda a lógica reativa do servidor.

## Estrutura da Aplicação
A aplicação está dividida em:
1.  **Carregamento Global:** Leitura inicial dos dados ao arrancar o processo Python.
2.  **Interface (UI):** Definição do layout, barra lateral, filtros e cartões.
3.  **Servidor:** Lógica de filtragem, renderização de gráficos e gestão de estado.

## Dependências Principais
* `shiny`: Framework web.
* `pandas`: Manipulação de dados.
* `matplotlib`: Geração de gráficos estáticos.
* `src`: Módulo local com funções auxiliares de leitura e validação.
"""
...
"""list: Lista crua de dicionários lida diretamente dos ficheiros na pasta data."""

ficheiros_invalidos = validar_dados(dados_brutos)
"""list: Lista contendo os nomes dos ficheiros que não passaram na validação inicial."""

dados_validos = [
    dado for dado in dados_brutos
    if dado.get("Origem") not in ficheiros_invalidos
]
"""list: Lista filtrada contendo apenas os dados dos ficheiros considerados válidos."""

pacote = converterParaPandas(dados_validos)
"""dict: Dicionário contendo os DataFrames 'cabecalho', 'movimentos' e 'rodape'."""
